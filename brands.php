...
<div id="side2" style="height:auto" class="container">
<?php
$n='';
$host='localhost';
$username='root';
$pwd='';
$sqlt="SELECT * FROM stores  WHERE REPLACE(store, ' ', '') = REPLACE('".$start."', ' ', '') ";
$recordst=mysqli_query($conn, $sqlt);


	while($couponlistingt = mysqli_fetch_assoc($recordst)){
		$n=$couponlistingt['store'];
		echo "<div style='width:100%;text-align:center;'><img src='../media/stores_logo/".$couponlistingt['logo']."'style='margin:0 auto;' width='100px'/></div>";
		echo "<div id='in-side21' style='width:100%;text-align:center;'>".$couponlistingt['store']."</div>";
		echo "<div id='in-side22'>".$couponlistingt['store_description']."</div>";
		}
		
		echo "<script>$('title').text('".$n." coupons | ".$n." promo code | ".$n." coupon codes  ');</script>";

echo "<script>$('#meta').attr('content','".$n." coupons, ".$n." promo code, ".$n." coupon codes, ".$couponlistingst['store']." promotional codes, ".$n."  ');</script>";
?>
...
<?php
$sqlt="SELECT * FROM couponlisting INNER JOIN stores ON couponlisting.store_name= stores.store where store_name='".$n."' ORDER BY rand()";
$recordst=mysqli_query($conn, $sqlt);
