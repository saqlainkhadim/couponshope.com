...
$sqltt="SELECT * FROM categories WHERE REPLACE(category, ' ', '') = REPLACE('".$start."', ' ', '') ";
$recordstt=mysqli_query($conn, $sqltt);


	while($couponlistingtt = mysqli_fetch_assoc($recordstt))

	{
	$n=$couponlistingtt['category'];
}

if($n==''){

$sqltt="SELECT * FROM categories WHERE category LIKE '".$st."%".$u."' ";
$recordstt=mysqli_query($conn, $sqltt);
...
echo "<script>$('title').text('".$n." Coupons- CouponShope')</script>";
...
<div id="side2"><?php echo $n?>
           <?php



$sqlt="SELECT * FROM couponlisting INNER JOIN stores ON couponlisting.store_name= stores.store where category='".$n."' ORDER BY rand()";
$recordst=mysqli_query($conn, $sqlt);
